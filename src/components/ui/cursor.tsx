...
function CursorProvider({ ref, children, ...props }: CursorProviderProps) {
  ...
  React.useEffect(() => {
    ...
    const updateRect = () => {
      lastRect.current = parent.getBoundingClientRect();
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      rafId.current = requestAnimationFrame(() => {
        const rect = lastRect.current || parent.getBoundingClientRect();
...
function CursorFollow({
  ...
  className,
  style,
  transition = { stiffness: 400, damping: 35, bounce: 0 },
  ...props
}: CursorFollowProps) {
    ...
    <AnimatePresence>
      {isActive && (
        <motion.div
          ...
          className={cn(
            'transform-[translate(-50%,-50%)] pointer-events-none z-[9998] absolute will-change-transform',
            className,
          )}
