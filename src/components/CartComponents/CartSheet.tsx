...
export function CartSheet({ className, children, fill }: CartSheetProps) {
  ...
  useEffect(() => {
    // Check authentication status and fetch cart items
          ...
          <div>
            <div className="relative md:flex hidden items-center justify-center cursor-pointer" onClick={handleCartClick}>
              <svg
                ...
                width="18" // bigger width
                height="22" // bigger height
                viewBox="0 0 14 18"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                strokeWidth="2" // makes it bolder
              >
                <path
              ...
              {cartItemCount > 0 && (
                <span className="absolute top-[6px] text-secondary-foreground text-[8px] font-semibold px-1.5 py-0.5 rounded-full">
                  {cartItemCount}
                </span>
              )}
            </div>
